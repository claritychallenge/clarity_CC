...
def audiogram(listener):
    """Construct audiogram from listener dict.

    Args:
        listener (dict): listener data

    Returns:
        Audiogram: 2-dim array of audiogram values (L, R)
            or levels, and a set of corresponding audiogram frequencies

    """
    # Construct audiogram ndarray
    return Audiogram(
        cfs=np.array(listener["audiogram_cfs"]),
        levels_l=np.array(listener["audiogram_levels_l"]),
        levels_r=np.array(listener["audiogram_levels_r"]),
    )
