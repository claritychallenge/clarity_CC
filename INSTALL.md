...
To download: click on the file to select it and then click on the download icon in the top-right of the interface. Select 'As individidual files'. (The site also lists datapacks for 'premixed' scenes. You do no need to download these as they will be generated by the software.)
...
IMPORTANT: Do not untar the package by hand. The `unpack.sh` script does more than a just untarring: it also adds a couple of important symbolic links between the data directory and the software directory, and copies a missing metadata file into the correct location. Read the script if you are unsure.
...
Note, the `predict.sh` will take a long time to run completely. For each scene, intelligibility predictions for three different listeners are generated. The script can be safely stopped at any stage and outputs for scenes processed so far will appear appended to a file called `sii.txt` in `<CLARITY_ROOT>/data/clarity_data/train/scenes`.
